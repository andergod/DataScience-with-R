...
The highest proportion of cancelled flights happens on February, while the lowest is on November. Most cancellations seems to ocurr at the end and beggining year and then they decreased the following months.
...
![](dsb2023/images/sfo-cancellations.png)
...
age_gaps %>% 
  group_by(movie_name) %>% 
  summarise(love_interest=n()) %>% 
  arrange(desc(love_interest)) %>% 
  top_n(1, love_interest)
```
